...
def run():
    ...
    print(f"  Fin propre FL21={results['turns']['pct_ended_fl21']}%  FL22={results['turns']['pct_ended_fl22']}%")
    for r in msg_ttests:
        print(f"  {r['label']:35s}  FL21={r['mean_fl21']:.3f}  FL22={r['mean_fl22']:.3f}  p={r.get('p','?')} {r.get('sig','')}")
    return results
